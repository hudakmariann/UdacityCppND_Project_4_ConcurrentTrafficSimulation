...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    //the below line does not really work with if. In c++ reference exampe, they use while. Also in Knowledge, while is suggested. But why...?
    while (_trafficLight.getCurrentPhase()==TrafficLight::TrafficLightPhase::red)
    {_trafficLight.waitForGreen();}  

    lck.lock();
    std::cout << "Intersection #" << _id << ": Vehicle #" << vehicle->getID() << " is granted entry." << " TrafficLight= " << _trafficLight.getCurrentPhase() << std::endl;
    lck.unlock();   
}
